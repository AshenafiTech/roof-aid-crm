// Deno-side copy of the canonical default sections per template kind.
// Kept in sync with apps/web/lib/templates/defaults.ts so the Edge
// Function can render even when no custom version is published.

import type { Block, InlineSpan, Section, TemplateDoc } from './template-pdf.ts'

const p = (text: string): Block => ({ type: 'paragraph', spans: [{ text }] })
...
function s(id: string, title: string, content: Block[]): Section {
  return { id: `default-${id}`, title, content }
}
...
function thirdPartyAuth(): Section[] {
  return [
    ...
    s('3pa-comm', 'Authorization to Communicate with Insurance Carrier', [
      p('The Homeowner authorizes {{contractor_name}} to communicate with the insurance company ONLY regarding:'),
...
function supplementDoc(): Section[] {
  return [
    s('sup-intro', 'Supplement Overview', [
      p('This supplement is attached to the homeowner contract dated {{today}}. Detailed line items are enumerated below.'),
    ]),
    s('sup-items', 'Line Items', [p('Add line items here.')]),
  ]
}

export type TemplateKind = '3rd_party_auth' | 'acv_contract' | 'rcv_contract' | 'supplement'
